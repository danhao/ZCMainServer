...
public class ListSelfDebtsAction extends PBBaseAction {
	...
	public void done(PBRequestSession reqSession, PBMessage request,
		...
		case TYPE_BID:
			...
			break;
		case TYPE_WIN:
			list = DebtService.getDebtDao().listDebts(DebtService.SIZE, (req.getPage() - 1) * DebtService.SIZE, "-publishTime", req.getState(), req.getType(), null, 0, 0, 0, 0, 0, 0, 0, 0, 0, reqSession.getPlayer().getWinDebts(), null, req.getDebtorName(), req.getDebtorId(), req.getProperty(), req.getHand(), req.getNewestMessage());
			break;
		}
